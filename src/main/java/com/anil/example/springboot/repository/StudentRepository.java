...
public interface StudentRepository extends JpaRepository<Student, Long> {
	...
	@Query("SELECT s FROM Student s WHERE CONCAT(s.status) LIKE %?1%")
	public List<Student> findThroughStatus(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.tAddress) LIKE %?1%")
	public List<Student> findBytAddress(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.lName) LIKE %?1%")
	public List<Student> findThroughlName(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.dob) LIKE %?1%")
	public List<Student> findThroughdob(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.gender) LIKE %?1%")
	public List<Student> findThroughgender(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.jobType) LIKE %?1%")
	public List<Student> findThroughjobType(String keyword);
	
	@Query("SELECT s FROM Student s WHERE CONCAT(s.jobRole) LIKE %?1%")
	public List<Student> findThroughjobRole(String keyword);


	@Query("SELECT s FROM Student s WHERE CONCAT(s.about) LIKE %?1%")
	public List<Student> findByabout(String keyword);
